...
if [ ! -n "$1" ] ;then
	echo "kubeadm init --image-repository registry.aliyuncs.com/google_containers --kubernetes-version=v$K8sVersion --pod-network-cidr=10.244.0.0/16"
	echo "Please wait a few minutes!"
	sudo kubeadm init --image-repository registry.aliyuncs.com/google_containers --kubernetes-version=v$K8sVersion --pod-network-cidr=10.244.0.0/16 > init.log
else
        echo "kubeadm init --image-repository registry.aliyuncs.com/google_containers --kubernetes-version=v$K8sVersion --apiserver-advertise-address $1 --pod-network-cidr=10.244.0.0/16"
        echo "Please wait a few minutes!"
	sudo kubeadm init --image-repository registry.aliyuncs.com/google_containers --kubernetes-version=v$K8sVersion --apiserver-advertise-address $1 --pod-network-cidr=10.244.0.0/16 > init.log
fi
...
if [ $? -eq 0 ]; then
    ...
    cp setupconfig.sh install-docker.sh  install-k8s.sh install-node.sh remove.sh /tmp/k8s-node-install
    cd /tmp
